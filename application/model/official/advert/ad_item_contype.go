...
func init() {
	Contype.AddItem(echo.NewKV(`text`, `文字广告`).SetFn(func(c context.Context) interface{} {
		return ContentRenderer(func(v Adverter) string {
			return `<a href="` + v.GetURL() + `" target="_blank">` + v.GetContent() + `</a>`
		})
	}))
	Contype.AddItem(echo.NewKV(`image`, `图片广告`).SetFn(func(c context.Context) interface{} {
		return ContentRenderer(func(v Adverter) string {
			return `<a href="` + v.GetURL() + `" target="_blank"><img rel="` + v.GetContent() + `" class="previewable" src="` + v.GetContent() + `"` + genStyle(v) + ` /></a>`
		})
	}))
	Contype.AddItem(echo.NewKV(`video`, `视频广告`).SetFn(func(c context.Context) interface{} {
		return ContentRenderer(func(v Adverter) string {
			return `<a href="` + v.GetURL() + `" target="_blank"><video src="` + v.GetContent() + `" controls="controls"` + genStyle(v) + `></video></a>`
		})
	}))
	Contype.AddItem(echo.NewKV(`audio`, `音频广告`).SetFn(func(c context.Context) interface{} {
		return ContentRenderer(func(v Adverter) string {
