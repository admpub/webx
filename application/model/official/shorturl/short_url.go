...
func NewShortURL(ctx echo.Context) *ShortURL {
	return &ShortURL{
		URL:    dbschema.NewOfficialShortUrl(ctx),
...
func (f *ShortURL) check() error {
	f.URL.LongUrl = strings.TrimSpace(f.URL.LongUrl)
	if len(f.URL.LongUrl) == 0 {
		return f.base.E(`请输入网址`)
	}
	pos := strings.Index(f.URL.LongUrl, `://`)
	if pos <= 0 {
		return f.base.E(`网址无效: %s`, f.URL.LongUrl)
	}
	switch strings.ToLower(f.URL.LongUrl[0:pos]) {
	case `http`, `https`:
	default:
		return errors.New(f.base.T(`网址无效: %s`, f.URL.LongUrl) + `, ` + f.base.T(`必须以“http://”或“https://”开头`))
	}
	f.URL.LongHash = com.Md5(f.URL.LongUrl)
	return nil
}
...
func (f *ShortURL) Add() (pk interface{}, err error) {
	...
	err = f.ExistsHash(f.URL.LongHash)
	if err != nil {
		if err != ErrLongURLExists {
			return
		}
		err = f.base.E(`网址已存在: %s`, f.URL.LongUrl)
		return
	}
	var urlInfo *url.URL
	...
	if err != nil {
		...
		f.Domain.OwnerId = f.URL.OwnerId
		f.Domain.OwnerType = f.URL.OwnerType
		f.Domain.UrlCount = 1
		_, err = f.Domain.Insert()
	} else {
		if f.Domain.Disabled == `Y` {
			err = f.base.E(`操作失败！域名“%v”已经被加入黑名单。如有疑问，请联系我们`, f.Domain.Domain)
			return
		}
	...
	f.URL.DomainId = f.Domain.Id
	f.URL.Visited = 0
	f.URL.Visits = 0
	if len(f.URL.ShortUrl) == 0 {
		f.URL.ShortUrl = ShortURLGenerator.Generate()
	}
	err = f.Exists(f.URL.ShortUrl)
	if err != nil {
		return
	}
	if len(f.URL.Password) > 0 {
		f.URL.Password = com.Md5(f.URL.Password)
	}
	pk, err = f.URL.Insert()
	return
}
...
func (f *ShortURL) Edit(mw func(db.Result) db.Result, args ...interface{}) (err error) {
	...
	err = f.ExistsHashOther(f.URL.LongHash, f.URL.Id)
	if err != nil {
		return
	}
	oldURL := dbschema.NewOfficialShortUrl(nil)
	oldURL.CPAFrom(f.URL)
	err = oldURL.Get(nil, args...)
	if err != nil {
		if err == db.ErrNoMoreRows {
			return f.base.E(`不存在ID为“%d”的数据`, f.URL.Id)
		}
		return
	}
	oldDomain := dbschema.NewOfficialShortUrlDomain(nil)
	oldDomain.CPAFrom(f.URL)
	...
	err = f.ExistsOther(f.URL.ShortUrl, f.URL.Id)
	if err != nil {
		return
	}
	modifyPassword := f.base.Formx(`modifyPassword`).Bool()
	if modifyPassword {
		if len(f.URL.Password) > 0 {
			f.URL.Password = com.Md5(f.URL.Password)
		} else {
		...
		f.URL.Password = oldURL.Password
	}
	err = f.URL.Update(mw, args...)
	return
}

func (f *ShortURL) Exists(shortURL string) error {
	exists, err := f.URL.Exists(nil, db.Cond{`short_url`: shortURL})
	if err != nil {
		return err
	}
	if exists {
		err = f.base.E(`短网址名称“%s”已经存在`, shortURL)
	}
	return err
}

func (f *ShortURL) ExistsHash(longHash string) error {
	exists, err := f.URL.Exists(nil, db.Cond{`long_hash`: longHash})
	if err != nil {
		return err
	}
	if exists {
...
func (f *ShortURL) ExistsHashOther(longHash string, id uint64) error {
	exists, err := f.URL.Exists(nil, db.Cond{`long_hash`: longHash}, db.Cond{`id`: db.NotEq(id)})
	if err != nil {
		return err
	}
	if exists {
...
func (f *ShortURL) ExistsOther(shortURL string, id uint64) error {
	exists, err := f.URL.Exists(nil, db.And(
...
func (f *ShortURL) Find(shortURL string) (longURL string, err error) {
	...
	t := time.Now()
	ts := uint(t.Unix())
	if f.URL.Expired > 0 && f.URL.Expired < ts {
		err = errors.WithMessage(ErrExpiredURL, f.base.T(`网址已经于“%s”过期`, time.Unix(int64(f.URL.Expired), 0).Format(`2006-01-02 15:04:05`)))
		return
	}
	if len(f.URL.Password) > 0 {
		inputPwd := f.base.Formx(`password`).String()
		if len(inputPwd) == 0 {
			err = ErrNeedURLPassword
			return
		}
		if f.URL.Password != com.Md5(inputPwd) {
			err = ErrWrongURLPassword
			return
		}
	}
	longURL = f.URL.LongUrl
	err = f.VisitAdd(t)
	if err != nil {
		return
	}
	err = f.URL.UpdateFields(nil, echo.H{
		`visited`: ts,
...
func (f *ShortURL) GenVisitData(t time.Time) error {
	f.Visit.UrlId = f.URL.Id
	f.Visit.DomainId = f.URL.DomainId
	f.Visit.OwnerId = f.URL.OwnerId
	f.Visit.OwnerType = f.URL.OwnerType
	f.Visit.Year = uint(t.Year())
	f.Visit.Month = uint(t.Month())
	f.Visit.Day = uint(t.Day())
	f.Visit.Hour = uint(t.Hour())
	f.Visit.Ip = f.base.RealIP()
	acceptLanguage := f.base.Header(`Accept-Language`)
	...
	f.Visit.Referer = f.base.Referer()
	...
	userAgent := f.base.Request().UserAgent()
...
func (f *ShortURL) VisitListWithURL(rows []*dbschema.OfficialShortUrlVisit) ([]*ShortURLVisitWithURL, error) {
	...
	for index, row := range rows {
		lastList[index] = &ShortURLVisitWithURL{
			...
			URL:                   dbschema.NewOfficialShortUrl(f.base.Context),
		}
	...
	if len(urlIDs) > 0 {
		_, err := f.URL.ListByOffset(nil, nil, 0, -1, db.Cond{`id`: db.In(urlIDs)})
		if err != nil {
			return lastList, err
		}
		for _, urlRow := range f.URL.Objects() {
