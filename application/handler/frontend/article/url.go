...
func Redirect(ctx echo.Context) error {
	...
	parts[0] = com.WithURLParams(parts[0], `_token`, config.FromFile().Encode256(url+`||`+time.Now().Format(time.DateTime)))
...
func verifyRedirectURL(ctx echo.Context) error {
	...
	if len(content) == 0 {
		data.SetError(ctx.NewError(code.DataFormatIncorrect, `令牌分析失败`))
		return ctx.JSON(data)
	}
	parts := strings.SplitN(content, `||`, 2)
	if len(parts) != 2 {
		data.SetError(ctx.NewError(code.DataFormatIncorrect, `令牌分析后的格式不正确`))
		return ctx.JSON(data)
	}
	if parts[0] != url {
		data.SetError(ctx.NewError(code.DataUnavailable, `令牌与网址不匹配`))
		return ctx.JSON(data)
	}
	...
	if t.Before(time.Now().Add(time.Minute * -5)) {
		data.SetError(ctx.NewError(code.DataHasExpired, `令牌已经过期`))
	} else {
